...
function mapEventRow(e: any, venueName: string): EventRow {
...
async function fetchUpcomingEventPages(venueIds: string[], offset: number): Promise<any[]> {
  const rows: any[] = [];
  let from = offset;
...
export default function VenueDetailScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (venueIdsToSearch.length > 0) {
        // events.venue_id and reviews.venue_id are UUID columns.
        // external_entity_ids returns non-UUID values (Setlist.fm numeric IDs,
        // etc.) that cause a PostgreSQL 22P02 cast error when passed to .in() —
        // the error is swallowed by `|| []`, silently showing nothing.
        const venueUuidsToSearch = venueIdsToSearch.filter(isUuid);
        ...
        setName(venueName || 'Venue');
        upcomingQueryRef.current = { ids: venueUuidsToSearch, name: venueName };
        ...
        setReviews(reviewRows.map((rv): NetworkReview => ({
          ...
          connection_degree: 1,
          event_info: {
            artist_name: rv.events?.artist_name ?? undefined,
  ...
  const onToggleFollow = useCallback(async () => {
    if (!sessionUserId || !venueId || followLoading) return;
    setFollowLoading(true);
    const next = !isFollowing;
    setIsFollowing(next);
    setFollowerCount(c => Math.max(0, next ? c + 1 : c - 1));
    const ok = await VenueFollowService.setVenueFollow(sessionUserId, venueId, next);
    if (!ok) {
      setIsFollowing(!next);
      setFollowerCount(c => Math.max(0, next ? c - 1 : c + 1));
    }
    setFollowLoading(false);
  }, [sessionUserId, venueId, isFollowing, followLoading]);
...
const styles = StyleSheet.create({
  ...
  hero: { width: '100%', height: 180, borderRadius: 16, marginBottom: 4 },
  statsRow: {
    flexDirection: 'row',
    gap: 16,
    flexWrap: 'wrap',
