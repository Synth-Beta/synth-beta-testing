...
export class SceneService {
  ...
  static async getUserSceneProgress(userId: string, sceneId: string): Promise<UserSceneProgress | null> {
    try {
      ...
      if (error) {
        ...
        if (error.code === 'PGRST301' || errStatus === 406) {
  ...
  static async getAllUserSceneProgress(userId: string): Promise<UserSceneProgress[]> {
    try {
      ...
      const fnErrStatus = (functionError as { status?: number } | null)?.status;
      if (functionError && fnErrStatus !== 406) {
      ...
      if (error) {
        ...
        const eSt = (error as { status?: number }).status;
        if (error.code === 'PGRST301' || eSt === 406) {
  ...
  static async getSceneDetails(sceneId: string, userId?: string): Promise<SceneDetail | null> {
    try {
      ...
      const eventQueries: PromiseLike<any>[] = [];
      ...
      if (participating_cities.length > 0) {
        eventQueries.push(
          supabase
            ...
            .order('event_date', { ascending: true })
            .limit(50)
            .then(({ data: cityEvents, error: cityErr }) => {
      ...
      if (participating_genres.length > 0) {
        eventQueries.push(
          supabase
            ...
            .order('event_date', { ascending: true })
            .limit(50)
            .then(({ data: genreEvents, error: genreErr }) => {
