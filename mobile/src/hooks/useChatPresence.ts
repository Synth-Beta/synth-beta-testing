 ...
 * no migration) so the channel is torn down when the screen unmounts. Mirrors
 * src/hooks/useChatPresence.ts on web; both sides share the same channel, so a
 * web user and a mobile user see each other typing.
 */
...
function useOwnDisplayName(userId: string | null): string {
