...
/**
 * Detect whether a username was auto-generated by an OAuth provider (e.g. Apple Sign In
 * with a hidden/private-relay email like 8qn7rx2pdq@privaterelay.appleid.com).
 *
 * Heuristics:
 *  1. Starts with a digit — human-chosen usernames almost never start with a number
 *  2. All lowercase alphanumeric with no separators (no _ or .) AND 10+ characters
 *     AND contains at least one digit — the hallmark of Apple private-relay username
 *     generation (e.g. "8qn7rx2pdq"). Real human names like "sloiterstein" are all
 *     letters and must NOT be flagged.
 */
export function isAutoGeneratedUsername(username: string): boolean {
  if (!username) return true;
  const lower = username.toLowerCase();
  // Starts with a digit — human-chosen usernames almost never do
  if (/^[0-9]/.test(lower)) return true;
  // Pattern: letters-then-digits (e.g. "tejpatel1510", "john99") — definitely human-chosen
  if (/^[a-z]{4,}[0-9]+$/.test(lower)) return false;
  // No separators, 10+ chars, AND contains at least one digit — hallmark of Apple
  // private-relay generated usernames (e.g. "8qn7rx2pdq" with digits interspersed)
  if (/^[a-z0-9]{10,}$/.test(lower) && /[0-9]/.test(lower)) return true;
  return false;
}
