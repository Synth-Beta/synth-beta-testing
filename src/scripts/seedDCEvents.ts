...
export class DCEventSeeder {
  ...
  static async saveEventsToSupabase(events: Event[]): Promise<number> {
    try {
      ...
      for (let i = 0; i < events.length; i += batchSize) {
        ...
        try {
          ...
          if (newEvents.length > 0) {
            ...
            console.log(`✅ Saved ${data?.length || 0} new events in this batch`);
            
            // Log some event titles for verification
            if (data && data.length > 0) {
              console.log(`📋 Sample events saved: ${data.slice(0, 3).map(e => e.title).join(', ')}${data.length > 3 ? '...' : ''}`);
            }
          } else {
            console.log('⏭️  No new events in this batch (all already exist)');
          }
      ...
      console.log(`🎉 Successfully saved ${savedCount} new events to Supabase!`);
      return savedCount;
  ...
  static async seedDCEvents(): Promise<{ fetched: number; saved: number }> {
    try {
      console.log('🚀 Starting DC Events Seeding Process...');
      console.log('📍 Location: Washington, DC');
      console.log('🎯 Target: 100 upcoming events');
      
      // Step 1: Fetch events from JamBase
      const events = await this.fetchDCEvents();
      console.log(`📊 Fetched ${events.length} events from JamBase`);
      ...
      console.log('✨ DC Events Seeding Complete!');
      console.log(`📈 Results: ${events.length} fetched, ${savedCount} saved`);
      
      return { fetched: events.length, saved: savedCount };
      
    } catch (error) {
      console.error('💥 DC Events Seeding Failed:', error);
      throw error;
    }
  }
}
