...
export function EventReviewModal({
    ...
    try {
      ...
      if (review) {
        ...
        setFormData({
          ...
          venueReviewText: '', // We'll need to parse this from review_text if needed
          artistReviewText: '', // We'll need to parse this from review_text if needed
        ...
        resetForm();
        // Set default review type to event
  ...
  const handleSubmit = async () => {
    ...
    if (formData.reviewType === 'event') {
      if (!formData.selectedArtist || !formData.selectedVenue || formData.artistRating === 0 || formData.venueRating === 0) {
        toast({
          title: "Incomplete Review",
          description: "Please select an artist, venue, and provide both artist and venue ratings.",
          variant: "destructive",
        });
        return;
      }
    } else if (formData.reviewType === 'venue') {
      if (!formData.selectedVenue || formData.venueRating === 0) {
        toast({
          title: "Incomplete Review",
          description: "Please select a venue and provide a venue rating.",
          variant: "destructive",
        });
        return;
      }
    } else if (formData.reviewType === 'artist') {
      if (!formData.selectedArtist || formData.artistRating === 0) {
        toast({
          title: "Incomplete Review",
          description: "Please select an artist and provide an artist rating.",
          variant: "destructive",
        });
        return;
      }
    ...
    setLoading(true);
    try {
      // Combine review texts into a single review_text field
      ...
      const reviewData: ReviewData = {
        ...
        artist_rating: formData.artistRating > 0 ? formData.artistRating : undefined,
        venue_rating: formData.venueRating > 0 ? formData.venueRating : undefined,
        rating: formData.rating, // This will be calculated automatically by the database
        review_text: combinedReviewText || undefined,
        reaction_emoji: formData.reactionEmoji || undefined,
        is_public: formData.isPublic,
        venue_tags: [], // TODO: Add venue tags from form
        artist_tags: [], // TODO: Add artist tags from form
      };

      console.log('🔍 Debug: About to submit review with data:', reviewData);
      console.log('🔍 Debug: userId:', userId, 'eventId:', eventId);
      
      // Get venue ID if we have a selected venue
      const venueId = formData.selectedVenue?.id || undefined;
