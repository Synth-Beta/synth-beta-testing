...
type ProfileTab = 'timeline' | 'interested' | 'passport';
...
export const MainApp = ({ onSignOut }: MainAppProps) => {
  ...
  useEffect(() => {
    ...
    // Separate query (and separate try/catch) from the username check above —
    // if `contact_email` doesn't exist yet (migration not applied), that failure
    // must not affect the unrelated username check.
    const checkContactEmail = async () => {
      try {
        ...
        console.warn('Contact email check failed:', err);
      }
    };
    ...
    >
      ...
      <SideMenu
        ...
        onNavigateToVerification={() => {
          handleNavigateToProfile(undefined, 'passport');
        }}
        onSignOut={handleSignOut}
