...
export async function ensurePublicUserProfile(): Promise<PublicUserRecoveryResult> {
  try {
    ...
    if (error) {
      console.error('[publicUserRecovery] RPC failed:', error);
      return {
        success: false,
        inserted: false,
        userId: null,
    ...
    if (reportedError) {
      console.warn('[publicUserRecovery] Server reported an error while ensuring profile:', reportedError);
    }
    ...
    console.error('[publicUserRecovery] Unexpected error:', err);
    return {
      success: false,
      inserted: false,
      userId: null,
