-- ============================================
-- FIX PROMOTION DETECTION IN PERSONALIZED FEED
-- ============================================
-- This migration fixes the JOIN logic to properly detect promoted events
-- and ensures they appear in the personalized feed
...
-- Create personalized feed function with FIXED promotion detection
...
BEGIN
  RETURN QUERY
  WITH base_events AS (
    -- Get all events with promotion data via LEFT JOIN
    SELECT
      e.id,
      e.jambase_event_id,
      e.title,
      ...
      -- PROMOTION DATA - FIXED JOIN LOGIC
...
-- Debug query to check for active promotions
CREATE OR REPLACE FUNCTION debug_active_promotions()
RETURNS TABLE(
  promotion_id UUID,
  event_id UUID,
  event_title TEXT,
...
GRANT EXECUTE ON FUNCTION debug_active_promotions() TO authenticated;

-- Verification
SELECT 
  'Fixed Promotion Detection in Personalized Feed' as status,
  'Genre-first algorithm with FIXED promotion detection. Promoted events will appear at positions 1, 6, 11, 16, 21...' as description;

-- Show current active promotions
SELECT 'Current Active Promotions:' as debug_info;
SELECT * FROM debug_active_promotions();
