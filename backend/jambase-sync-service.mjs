...
class JambaseSyncService {
  ...
  async upsertEvents(eventsData) {
    ...
    // CRITICAL: never delink an event from its artist/venue. When a link UUID could
    // not be resolved this run, omit the column so upsert keeps the existing FK on
    // update (and inserts null by default for genuinely new events). Writing null
    // here would clobber correct links and break artist/venue listings + images.
    const safeEventsData = eventsData.map((row) => {
      const next = { ...row };
      if (!next.artist_id) delete next.artist_id;
      if (!next.venue_id) delete next.venue_id;
      return next;
    });
    ...
    try {
      const { error } = await this.supabase
        .from('events')
        .upsert(safeEventsData, { onConflict: 'jambase_id', ignoreDuplicates: false });
