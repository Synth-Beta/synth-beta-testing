-- ============================================
-- FIX FEED JOIN LOGIC - SHOW ALL EVENTS
-- ============================================
-- This fixes the JOIN logic to show all events, not just promoted ones
...
-- Create personalized feed function with FIXED JOIN logic
...
-- Test the fixed feed
SELECT 
  'Fixed Feed Test - Should show ALL events:' as status;
...
SELECT 
  'Promoted events:' as info,
  COUNT(*) as promoted_count
FROM jambase_events 
WHERE is_promoted = true AND event_date >= CURRENT_DATE;

-- Test the feed function
SELECT 
  'Feed test results:' as info,
  COUNT(*) as total_in_feed,
  COUNT(CASE WHEN is_promoted = true THEN 1 END) as promoted_in_feed,
  COUNT(CASE WHEN is_promoted = false OR is_promoted IS NULL THEN 1 END) as regular_in_feed
