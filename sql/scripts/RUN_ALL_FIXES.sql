...
-- ============================================
-- This file combines all fixes in the correct order
...
-- Step 1.1: Merge data from newer venue into older venue
...
-- Step 1.2: Update all foreign key references from merge venue to keep venue
...
-- Update user_venues table (handle duplicates)
...
-- Update user_reviews table
UPDATE public.user_reviews
SET venue_id = '725a0c7f-dfd2-43e6-90e5-0c690eb377e4', updated_at = NOW()
WHERE venue_id = '8ec7a9e9-a4e8-4a1d-8cbe-77734a86a001';

-- Update user_venue_interactions table
...
-- Step 1.3: Recalculate num_upcoming_events for the kept venue
...
-- Step 1.4: Delete the duplicate venue
...
-- ============================================
-- Fixes the 5 Leif Totusek events that have null venue_id
...
-- Verify no references to deleted venue remain
SELECT 
