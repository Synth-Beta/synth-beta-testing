-- ============================================
-- CORRECTED GENRE-FIRST ALGORITHM FOR PROMOTIONS
-- ============================================
-- Based on actual database schema audit
-- Uses only tables and columns that actually exist
...
BEGIN
  ...
  WITH genre_signals AS (
    ...
    -- From interested events' genres (using user_jambase_events)
    SELECT 
    ...
    WHERE uje.user_id = p_user_id
      AND uje.interested = true  -- Using the actual column name
...
-- Verification
SELECT 
  'Genre-First Algorithm Fixed for Promotions (Corrected)' as status,
  'Uses only actual database schema - should work without errors' as description;
