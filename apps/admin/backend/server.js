...
// Supabase Configuration — require env (no hardcoded keys)
...
if (!process.env.SUPABASE_ANON_KEY && !process.env.SUPABASE_SERVICE_ROLE_KEY) {
  console.warn('Warning: SUPABASE_ANON_KEY / SUPABASE_SERVICE_ROLE_KEY not set');
}
if (!process.env.SUPABASE_ANON_KEY && process.env.SUPABASE_SERVICE_ROLE_KEY) {
...
// JamBase API
process.env.JAMBASE_API_KEY = process.env.JAMBASE_API_KEY || '';
