-- Fix Artist/Venue Relationships - Works with Existing Infrastructure
-- This SQL works with your current database schema and adds only what's missing

-- Step 1: Add missing artist_id column to user_reviews table (venue_id already exists)
...
-- Step 7: Create function to populate venue_id in user_reviews (if not already populated)
...
-- Step 8: Create enhanced view that works with existing public_reviews_with_profiles
...
-- Step 10: Create function to get events for an artist (for the artist card)
...
-- Step 11: Create function to get events for a venue (for the venue card)
...
-- Step 13: Create triggers to automatically populate artist_id and venue_id when reviews are created
