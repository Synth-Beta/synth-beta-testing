...
export class ReviewService {
  ...
  static async setEventReview(
    ...
    try {
      ...
      // Resolve artist_id and venue_id UUIDs from the event's artist_uuid and venue_uuid
      let artistIdForReview: string | undefined = undefined;
      try {
        ...
        if (eventData?.artist_uuid) {
          artistIdForReview = eventData.artist_uuid;
        }
        
        // If venue not provided but event has one, use it as fallback
        if (!venueId && eventData?.venue_uuid) {
          venueId = eventData.venue_uuid;
        }
      } catch (err) {
        console.warn('Could not resolve artist/venue UUIDs from event:', err);
      }
      ...
      if (existingReview) {
        ...
        const fullUpdate: any = {
          ...(artistIdForReview ? { artist_id: artistIdForReview } : {}),
