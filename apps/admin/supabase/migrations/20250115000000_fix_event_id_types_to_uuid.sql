-- Fix event_id data types to use UUID consistently
-- This migration aligns the database schema with TypeScript types

-- First, check if we have any data that would conflict
-- If there are existing records, we need to handle them carefully

-- Drop and recreate event_interests with UUID event_id
...
-- Drop and recreate user_swipes with UUID event_id
...
-- Drop and recreate matches with UUID event_id
