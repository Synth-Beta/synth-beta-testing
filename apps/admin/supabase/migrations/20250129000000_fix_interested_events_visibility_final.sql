-- Final fix for interested events visibility issues
-- This migration addresses all the problems preventing users from seeing:
-- 1. Other users' interested events on profiles
-- 2. Users interested in events on event details
...
-- This allows authenticated users to see all interested events (necessary for app functionality)
...
-- Step 5: Create a helper function to get users interested in an event
...
-- Step 7: Ensure all necessary permissions are granted
...
COMMENT ON FUNCTION public.get_user_interested_events(uuid) IS 'Returns all events that a user is interested in, with full event details';
COMMENT ON FUNCTION public.get_users_interested_in_event(uuid) IS 'Returns all users who are interested in a specific event, with profile details';
