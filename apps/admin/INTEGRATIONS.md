...
## 7. Instagram Platform API (Edge Function)

### Overview
Supabase runs the `instagram-analytics` Edge Function inside `supabase/functions/instagram-analytics/index.ts`. The function now calls `https://graph.instagram.com/v{version}/{instagram_user_id}` with the **Instagram Platform API token** generated via the Instagram use case (Instagram Login). It no longer touches `graph.facebook.com` or the Facebook Page–based tokens, so this works with the newer scopes your app exposes.

### Configuration
Store these secrets in Supabase (or `.env.local` for local development):

```env
INSTAGRAM_USER_ID=17841400000000000
INSTAGRAM_ACCESS_TOKEN=your_long_lived_instagram_platform_token
INSTAGRAM_APP_ID=your_instagram_app_id
INSTAGRAM_APP_SECRET=your_instagram_app_secret
INSTAGRAM_API_VERSION=v24.0  # optional; defaults to v24.0
```

`INSTAGRAM_USER_ID` must be the Instagram Business/Creator account ID (not the username). `INSTAGRAM_ACCESS_TOKEN` should be the **long-lived token generated through the Instagram use case → Generate access tokens → Instagram Tester** flow. `INSTAGRAM_APP_ID` and `INSTAGRAM_APP_SECRET` come from the Instagram App (not a Facebook App) under API Setup with Instagram Login. `INSTAGRAM_API_VERSION` lets you pin a specific vX.Y release (defaults to `v24.0`).
...
Instagram handles these scopes through the Instagram use case; you do not need any of the legacy `pages_*` permissions such as `pages_show_list` or `pages_read_engagement`.

### Token Generation Steps
1. In the [Meta Developer Dashboard](https://developers.facebook.com/apps/), create an Instagram App (app type **Business**) and add the **Instagram Platform** product.
2. Go to **Instagram → API Setup with Instagram Login** and configure your OAuth redirect URI (matching your frontend or dev callback). Under **Instagram use case**, click **Generate access tokens** → **Instagram Tester** and add the Professional account you want to monitor.
3. Copy the short-lived access token that the tester flow returns. Confirm it includes the permissions listed above and that the account type is Business or Creator. The same flow already gives you the user ID for that account.
4. Exchange the short-lived token for a long-lived one:
   ```bash
   curl "https://graph.instagram.com/access_token?grant_type=ig_exchange_token&client_secret=INSTAGRAM_APP_SECRET&access_token=SHORT_LIVED_TOKEN"
   ```
5. Store the long-lived token and corresponding `INSTAGRAM_USER_ID` in Supabase secrets. Long-lived tokens last ~60 days. Refresh them before expiration with:
   ```bash
   curl "https://graph.instagram.com/refresh_access_token?grant_type=ig_refresh_token&access_token=LONG_LIVED_TOKEN"
   ```

### Verification (Graph API Explorer / curl)
Before dropping the token into Supabase, confirm it works:

- **Profile check**:
  ```
  GET https://graph.instagram.com/v24.0/{INSTAGRAM_USER_ID}?fields=id,username,followers_count,media_count&access_token=LONG_LIVED_TOKEN
  ```
  The response must return the account metadata to prove the token and user ID pair are valid.

- **Insights smoke test** (requires `instagram_business_manage_insights`):
  ```
  GET https://graph.instagram.com/v24.0/{INSTAGRAM_USER_ID}/insights?metric=reach,impressions&period=day&access_token=LONG_LIVED_TOKEN&since=<unix_timestamp>
  ```
  Expect hourly/daily time-series data for impressions/reach. The Edge Function makes the same call for totals, so verifying it succeeds is critical.

### Notes
- The Instagram Platform API no longer returns a `profile_views` time series (see RapidDev's 2026 automation guide). The Edge Function logs a warning and falls back to `0` whenever that metric is missing.
- The Edge Function opts into `https://graph.instagram.com` per the current version, so any token scoped to the Instagram use case works — you no longer need Facebook Page access.
- Keep your App Secret and long-lived token sensitive; rotate them regularly and avoid committing secrets.
...
- **LinkedIn org posting**: use `LINKEDIN_ACCESS_TOKEN` + `LINKEDIN_ORG_ID` with `content-calendar-publish` (v1 is copy-ready drafts only; see [CONTENT_CALENDAR.md](./CONTENT_CALENDAR.md))

## Content Calendar / Editorial (DC)

See [CONTENT_CALENDAR.md](./CONTENT_CALENDAR.md).
...
NEWS_API_KEY=
CONTENT_CALENDAR_CRON_SECRET=
SYNTH_BRAND_IMAGE_URL=
LINKEDIN_ACCESS_TOKEN=
LINKEDIN_ORG_ID=
```

Instagram publishing reuses existing Meta secrets (`INSTAGRAM_USER_ID`, `INSTAGRAM_ACCESS_TOKEN`).
