...
// Configuration – load from .env.local when running: node --env-file=.env.local scripts/enrich-reviews-with-setlist-fm.js
const SETLIST_FM_API_KEY = process.env.SETLIST_FM_API_KEY;
if (!SETLIST_FM_API_KEY) {
  console.error('SETLIST_FM_API_KEY is not set. Add it to .env.local or run with: node --env-file=.env.local scripts/enrich-reviews-with-setlist-fm.js');
  process.exit(1);
}
const SETLIST_FM_BASE_URL = 'https://api.setlist.fm/rest/1.0';
// Security: Load Supabase credentials from env only — never commit service_role keys.
const SUPABASE_URL = process.env.SUPABASE_URL;
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SERVICE_KEY;
if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error('Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY. Add to .env.local (see .env.example).');
  process.exit(1);
}
