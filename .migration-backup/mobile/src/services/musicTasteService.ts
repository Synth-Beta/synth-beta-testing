...
function generateDescription(topArtists: string[], topGenres: string[]): string {
  if (topArtists.length === 0 && topGenres.length === 0) {
    return 'No listening data yet. Connect a streaming service to showcase your music taste.';
  }
  ...
  static async getUserMusicTaste(userId: string): Promise<MusicTasteSummary> {
    try {
      ...
      (interactions || []).forEach(row => {
        ...
        if ((row as { entity_type?: string }).entity_type === 'artist') {
          const name = String(md.name || 'Unknown Artist');
          const key = name.toLowerCase();
          const prev = artistCounts.get(key) || { name, count: 0, popularity: md.popularity as number | undefined };
          artistCounts.set(key, {
            name,
            count: prev.count + 1,
            popularity: (prev.popularity ?? md.popularity) as number | undefined,
          });
          const genres: string[] = Array.isArray(md.genres) ? (md.genres as string[]) : [];
          genres.map(normalizeGenre).forEach(g => genreCounts.set(g, (genreCounts.get(g) || 0) + 1));
        }
        if ((row as { entity_type?: string }).entity_type === 'track') {
          const artistNames: string[] = Array.isArray(md.artistNames) ? (md.artistNames as string[]) : [];
      ...
      return {
        ...
        topGenres: [],
        description:
          'No listening data yet. Connect a streaming service to showcase your music taste.',
        totalListeningHours: undefined,
        serviceType: 'unknown',
        lastUpdated: null,
      };
