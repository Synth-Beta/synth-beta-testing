-- ============================================
-- Shared genres with match percentages
-- ============================================
-- Returns shared_genres as objects with { genre, match_pct } instead of plain strings.
-- match_pct = geometric mean of both users' normalised genre weights:
--   sqrt( (user_a_score / user_a_total) * (user_b_score / user_b_total) ) * 100
-- This is high when BOTH users care about the genre, low when only one does.
-- ============================================
...
BEGIN
  ...
  -- Shared genres with match percentage (geometric mean of normalised weights)
  WITH scores1 AS (
...
COMMENT ON FUNCTION public.get_new_friend_celebration_data(uuid) IS 'Returns events attended, shared genres (with match_pct), shared artists/venues, suggested events with source labels.';
