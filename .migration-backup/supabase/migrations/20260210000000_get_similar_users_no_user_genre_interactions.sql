-- ============================================================
-- get_similar_users_to_friend: remove dependency on user_genre_interactions.
-- Uses only artist_follows, user_venue_relationships, and music_preference_signals
-- so the RPC works when user_genre_interactions does not exist.
-- ============================================================
...
COMMENT ON FUNCTION public.get_similar_users_to_friend(UUID, INT) IS
  'Returns users to add as friends, ranked by shared artists, venues, and genres (music_preference_signals only). Does not use user_genre_interactions. Excludes friends, blocked, and pending requests.';
