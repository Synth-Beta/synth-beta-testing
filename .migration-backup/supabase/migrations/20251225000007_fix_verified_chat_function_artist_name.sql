-- ============================================================
-- Fix get_or_create_verified_chat function
-- ============================================================
-- The error "column e.artist_name does not exist" suggests
-- the query might be accessing events.artist_name directly.
-- This migration ensures the function correctly joins with
-- artists and venues tables to get names.
-- Also optimizes queries to prevent timeouts by using indexes properly.

-- Create indexes on identifier columns for faster lookups
CREATE INDEX IF NOT EXISTS idx_artists_identifier ON public.artists(identifier) WHERE identifier IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_venues_identifier ON public.venues(identifier) WHERE identifier IS NOT NULL;
...
SET statement_timeout = '10s'  -- Prevent function from hanging
AS $$
DECLARE
  v_chat_id UUID;
  v_chat_name TEXT;
...
-- Comment
COMMENT ON FUNCTION public.get_or_create_verified_chat IS 'Gets existing verified chat or creates a new one for the specified entity. Fixed to properly join with artists/venues tables instead of accessing non-existent columns. Optimized for performance with proper index usage and 10s timeout.';
