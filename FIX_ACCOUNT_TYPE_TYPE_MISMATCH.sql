-- ============================================
-- QUICK FIX: account_type Type Mismatch Error
-- ============================================
-- Original Error: "Returned type account_type does not match expected type text in column 19"
-- Additional Error: "cannot change data type of view column 'photos' from jsonb to text[]"
-- 
-- Problem: The reviews_with_connection_degree view has incorrect types:
--          1. account_type (enum) should be TEXT
--          2. photos (jsonb) should be TEXT[] to match table schema
--
-- Solution: Drop and recreate the view with correct types
-- ============================================

-- Drop the existing view first (required to change column types)
DROP VIEW IF EXISTS public.reviews_with_connection_degree CASCADE;

-- Recreate the view with correct types
...
SELECT 
  ...
  ur.is_public,
  ur.is_draft,
  ur.photos::TEXT[] as photos,  -- Ensure photos is TEXT[] (not jsonb)
  ...
  p.account_type::TEXT as reviewer_account_type,  -- ✅ FIX: Cast enum to TEXT
  -- Event information
...
-- Update comment
COMMENT ON VIEW public.reviews_with_connection_degree IS 'Reviews from 1st, 2nd, and relevant 3rd degree connections. FIXED: account_type cast to TEXT and photos as TEXT[] to resolve type mismatch errors.';

-- Recreate the RPC function (was dropped by CASCADE)
...
RETURNS TABLE (
  ...
  is_public BOOLEAN,
  is_draft BOOLEAN,
  photos JSONB,  -- Function returns JSONB for compatibility
  setlist JSONB,
  likes_count INTEGER,
