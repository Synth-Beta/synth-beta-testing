...
| **Tertiary** | 22px | `--brand-pink-050` | `--brand-pink-500` | 2px `--brand-pink-500` | Labels, chips, tags |
...
- **Height**: 
  ...
  - Tertiary buttons: `22px` - chip style for labels, tags, and lightweight inline actions
...
- **Padding**: `12px` above and below header content (`var(--spacing-small, 12px)`)
- **Total Visual Height**: `68px` (44px content + 12px top + 12px bottom)
...
- **Icons**: `var(--brand-pink-500)` for active, `var(--neutral-600)` for inactive
- **Safe Area**: Add `env(safe-area-inset-bottom, 0px)` BELOW bottom nav
- **Content Spacing**: `32px` (`var(--spacing-bottom-nav, 32px)`) space between end of content and bottom of page
  - Content should end `32px` above the bottom of the page
  - This spacing is intentionally asymmetric with top spacing (12px below header) to maintain visual hierarchy
...
| Size | Value | Use Case |
|------|-------|----------|
| **Small** | 16px | Small icons (clear buttons, inline icons) |
...
- **Tertiary Buttons (Chips)**: `22px` tall, intended for labeling and tagging. Most chips are non-clickable labels. Clickable chips should be used sparingly and must be comfortably tappable through padding and spacing. Not for primary or destructive actions.
