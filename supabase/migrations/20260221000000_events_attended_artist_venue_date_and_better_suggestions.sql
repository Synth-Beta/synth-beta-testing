-- ============================================
-- 1. Events attended together: match on artist_id, venue_id, date (not just event_id)
--    Catches same real-world show when event records differ (e.g. duplicate ingestion).
-- 2. Better suggested events: add "attended_together" tier, require min genre weight.
-- ============================================
...
BEGIN
  ...
  SELECT MIN(ur.updated_at) INTO v_friends_since
  FROM public.user_relationships ur
  WHERE ur.relationship_type = 'friend' AND ur.status = 'accepted'
    AND ((ur.user_id = v_current_user_id AND ur.related_user_id = p_friend_id)
         OR (ur.user_id = p_friend_id AND ur.related_user_id = v_current_user_id));
  v_friendship_days := GREATEST(0, COALESCE((CURRENT_DATE - v_friends_since::date), 0)::int);
  ...
  -- Events attended together: match on artist_id, venue_id, and date
  -- Path A: reviews with event_id -> match via events table
  -- Path B: reviews with event_id IS NULL -> match directly on r.artist_id, r.venue_id, r."Event_date"
  SELECT COALESCE(jsonb_agg(rec ORDER BY rec->>'event_date' DESC NULLS LAST), '[]'::jsonb)
  INTO v_events_attended
  FROM (
    -- Path A: event_id-based reviews
    ...
    FROM (
      SELECT DISTINCT ON (e1.artist_id, e1.venue_id, (e1.event_date)::date) e1.id AS event_id
      FROM public.reviews r1
      INNER JOIN public.events e1 ON e1.id = r1.event_id AND e1.artist_id IS NOT NULL AND e1.venue_id IS NOT NULL
      INNER JOIN public.reviews r2 ON r2.user_id = p_friend_id AND r2.event_id IS NOT NULL AND r2.is_draft = false
      INNER JOIN public.events e2 ON e2.id = r2.event_id
        AND e1.artist_id = e2.artist_id
        AND e1.venue_id = e2.venue_id
        AND (e1.event_date)::date = (e2.event_date)::date
      WHERE r1.user_id = v_current_user_id AND r1.event_id IS NOT NULL AND r1.is_draft = false
    ...
    -- Path B: event_id IS NULL reviews - match on artist_id, venue_id, date from reviews
    -- Allow dates within 30 days (handles data-entry differences for same show)
    SELECT jsonb_build_object(
      'id', 'review-match-' || pb.rec_key,
      'title', a.name || ' at ' || v.name,
    ...
    FROM (
      SELECT DISTINCT ON (r1.artist_id, r1.venue_id, LEAST((r1."Event_date")::date, (r2."Event_date")::date))
        r1.artist_id,
        r1.venue_id,
        md5(r1.artist_id::text || r1.venue_id::text || LEAST((r1."Event_date")::date, (r2."Event_date")::date)::text) AS rec_key,
        LEAST(r1."Event_date", r2."Event_date") AS event_date
      FROM public.reviews r1
      INNER JOIN public.reviews r2 ON r2.user_id = p_friend_id AND r2.is_draft = false
      ...
      WHERE r1.user_id = v_current_user_id AND r1.event_id IS NULL AND r1.is_draft = false
        AND r1.artist_id IS NOT NULL AND r1.venue_id IS NOT NULL AND r1."Event_date" IS NOT NULL
        AND (r1.was_there = true OR (r1.review_text IS NOT NULL AND r1.review_text != 'ATTENDANCE_ONLY'))
        AND (r2.was_there = true OR (r2.review_text IS NOT NULL AND r2.review_text != 'ATTENDANCE_ONLY'))
    ) pb
    LEFT JOIN public.artists a ON a.id = pb.artist_id
    LEFT JOIN public.venues v ON v.id = pb.venue_id
  ) combined;

  -- Shared genres with match percentage
  WITH scores1 AS (
  ...
  total1 AS (
    SELECT COALESCE(NULLIF((SELECT SUM(v::numeric) FROM jsonb_each_text(s.val) AS kv(k, v)), 0), 1) AS total
    FROM scores1 s
  ),
  total2 AS (
    SELECT COALESCE(NULLIF((SELECT SUM(v::numeric) FROM jsonb_each_text(s.val) AS kv(k, v)), 0), 1) AS total
  ...
  shared AS (
    SELECT g.genre,
           COALESCE((s1.val->>g.genre)::numeric, 0) / t1.total AS pct1,
           COALESCE((s2.val->>g.genre)::numeric, 0) / t2.total AS pct2
    FROM (
    ...
    CROSS JOIN scores1 s1
    CROSS JOIN scores2 s2
    CROSS JOIN total1 t1
    CROSS JOIN total2 t2
  )
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'genre', genre,
        'match_pct', LEAST(ROUND(SQRT(pct1 * pct2) * 100), 99)
      )
      ORDER BY SQRT(pct1 * pct2) DESC, genre
    ),
    '[]'::jsonb
  ),
  COALESCE(ARRAY_AGG(genre ORDER BY SQRT(pct1 * pct2) DESC, genre), '{}')
  ...
  -- Suggested events: (1) attended_together_artists/venues, (2) you_both_follow, (3) recommended (min weight), (4) fallback
  WITH
  -- Artists BOTH users have attended (from reviews)
  attended_artists AS (
    SELECT DISTINCT e.artist_id FROM public.reviews r
    INNER JOIN public.events e ON e.id = r.event_id AND e.artist_id IS NOT NULL
    WHERE r.user_id = v_current_user_id AND r.event_id IS NOT NULL AND r.is_draft = false
      AND (r.was_there = true OR (r.review_text IS NOT NULL AND r.review_text != 'ATTENDANCE_ONLY'))
    INTERSECT
    SELECT DISTINCT e.artist_id FROM public.reviews r
    INNER JOIN public.events e ON e.id = r.event_id AND e.artist_id IS NOT NULL
    WHERE r.user_id = p_friend_id AND r.event_id IS NOT NULL AND r.is_draft = false
      AND (r.was_there = true OR (r.review_text IS NOT NULL AND r.review_text != 'ATTENDANCE_ONLY'))
  ),
  -- Venues BOTH users have attended
  attended_venues AS (
    SELECT DISTINCT e.venue_id FROM public.reviews r
    INNER JOIN public.events e ON e.id = r.event_id AND e.venue_id IS NOT NULL
    WHERE r.user_id = v_current_user_id AND r.event_id IS NOT NULL AND r.is_draft = false
      AND (r.was_there = true OR (r.review_text IS NOT NULL AND r.review_text != 'ATTENDANCE_ONLY'))
    INTERSECT
    SELECT DISTINCT e.venue_id FROM public.reviews r
  ...
  attended_together_events AS (
    ...
    WHERE e.event_date BETWEEN v_min_ts AND v_max_ts
      ...
      AND (
        (e.artist_id IN (SELECT artist_id FROM attended_artists))
        OR (e.venue_id IN (SELECT venue_id FROM attended_venues))
      )
  ...
  following_overlap AS (
    SELECT f1.eid FROM following1 f1 INNER JOIN following2 f2 ON f1.eid = f2.eid
  ),
  rec_intersection AS (
    SELECT w1.eid, (w1.total_weight + w2.total_weight) AS combined_weight
    FROM weights1 w1 
    INNER JOIN weights2 w2 ON w1.eid = w2.eid
    WHERE (w1.total_weight + w2.total_weight) >= 2.5
  ),
  combined AS (
    SELECT ate.eid, 0 AS sort_rank, ev.event_date, 0::numeric AS combined_weight, 'you_both_attended'::text AS source
    FROM attended_together_events ate 
    INNER JOIN public.events ev ON ev.id = ate.eid
    UNION ALL
    ...
    WHERE fo.eid NOT IN (SELECT eid FROM attended_together_events)
    UNION ALL
    SELECT ri.eid, 2 AS sort_rank, ev.event_date, ri.combined_weight, 'recommended'::text AS source
    FROM rec_intersection ri 
    INNER JOIN public.events ev ON ev.id = ri.eid
    WHERE ri.eid NOT IN (SELECT eid FROM following_overlap)
      AND ri.eid NOT IN (SELECT eid FROM attended_together_events)
  ),
  ordered AS (
    SELECT eid, sort_rank, event_date, combined_weight, source 
    FROM combined 
  ...
  fallback_events AS (
    ...
    WHERE e.event_date BETWEEN v_min_ts AND v_max_ts
      AND (NOT v_has_location OR (e.latitude IS NOT NULL AND e.longitude IS NOT NULL AND e.latitude BETWEEN v_min_lat AND v_max_lat AND e.longitude BETWEEN v_min_lng AND v_max_lng))
      AND ((array_length(v_shared_genres_arr, 1) > 0 AND e.genres && v_shared_genres_arr) 
    ...
    (SELECT COALESCE(
       jsonb_agg(
         jsonb_build_object(
           'id', ev2.id, 
           'title', ev2.title, 
           'event_date', ev2.event_date, 
           'venue_city', ev2.venue_city, 
           'venue_name', vn2.name, 
           'artist_name', ar2.name, 
           'genres', ev2.genres, 
           'source', 'fallback'
         ) 
  ...
  RETURN jsonb_build_object(
    ...
    'friendship_days', COALESCE(v_friendship_days, 0)
  );
END;
$func$;

COMMENT ON FUNCTION public.get_new_friend_celebration_data(uuid) IS 'Events attended (artist+venue+date match), shared genres, shared artists/venues, suggested events (attended_together, you_both_follow, recommended min weight 2.5, fallback), friendship_days.';
