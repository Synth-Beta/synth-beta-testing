...
BEGIN
  ...
  candidates AS (
    ...
    FROM (
      ...
      UNION
      -- Shared venues (venue_follows)
      SELECT vf2.user_id
      FROM venue_follows vf1
      JOIN venue_follows vf2 ON vf1.venue_name = vf2.venue_name
        AND COALESCE(vf1.venue_city, '') = COALESCE(vf2.venue_city, '')
        AND COALESCE(vf1.venue_state, '') = COALESCE(vf2.venue_state, '')
        AND vf2.user_id != p_user_id
  ...
  scored AS (
    ...
    LEFT JOIN (
      SELECT vf2.user_id, COUNT(DISTINCT (vf1.venue_name, COALESCE(vf1.venue_city,''), COALESCE(vf1.venue_state,'')))::INT AS cnt
      FROM venue_follows vf1
      JOIN venue_follows vf2 ON vf1.venue_name = vf2.venue_name
        AND COALESCE(vf1.venue_city,'') = COALESCE(vf2.venue_city,'')
        AND COALESCE(vf1.venue_state,'') = COALESCE(vf2.venue_state,'')
      WHERE vf1.user_id = p_user_id
      GROUP BY vf2.user_id
