...
BEGIN
  ...
  -- Event cluster scores (user affinity for event clusters)
  event_cluster_scores AS (
    SELECT
      ec.event_id,
      COALESCE(SUM(uca.score), 0)::NUMERIC AS cluster_score
    FROM event_clusters ec
    JOIN user_cluster_affinity uca ON uca.user_id = p_user_id
      AND uca.cluster_path_slug = ec.cluster_path_slug
      AND (uca.country IS NOT DISTINCT FROM ec.country)
    GROUP BY ec.event_id
  ),
  
  -- User umbrella genre IDs (expand genre_preferences keys to all genres under same umbrella)
  user_umbrella_genre_ids AS (
    SELECT DISTINCT ug.genre_id
    FROM user_pref up, jsonb_each(up.genre_preferences) g
    JOIN genres g0 ON g0.normalized_key = lower(regexp_replace(trim(g.key::TEXT), '[-_\s]+', ' ', 'g'))
    JOIN genre_paths gp ON gp.genre_id = g0.id
    CROSS JOIN LATERAL (SELECT * FROM get_genres_under_umbrella(split_part(gp.path_slug, '.', 1), 5)) ug(genre_id)
  ),
  
  -- Event scoring (optimized with precomputed artist follows + cluster + umbrella genre)
  scored_events AS (
    SELECT
      ec.*,
      (
        ...
        + LEAST(COALESCE(ecs.cluster_score, 0) * 2::NUMERIC, 20::NUMERIC)
        + CASE WHEN EXISTS (
            SELECT 1 FROM events_genres eg
            WHERE eg.event_id = ec.event_id
              AND eg.genre_id IN (SELECT * FROM user_umbrella_genre_ids)
          ) THEN 15::NUMERIC ELSE 0::NUMERIC END
      )::NUMERIC AS raw_score
    FROM event_candidates ec
    LEFT JOIN event_cluster_scores ecs ON ecs.event_id = ec.event_id
  ),
