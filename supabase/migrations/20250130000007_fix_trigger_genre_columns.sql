-- Fix the trigger to use the correct column names for user_genre_interactions
-- The table uses interaction_count instead of interaction_strength
...
BEGIN
  ...
  IF v_event_record.venue_name IS NOT NULL THEN
    INSERT INTO user_venue_interactions (
      user_id,
      venue_id,           -- This should be NULL since we don't have UUID
      venue_name,
      interaction_type,
      interaction_strength,
      source_entity_type,
      ...
      jsonb_build_object(
        ...
        'venue_id', v_event_record.venue_id,  -- Store JamBase venue_id in metadata
        'tour_name', v_event_record.tour_name,
        'event_date', v_event_record.event_date
      ),
      NOW()
  ...
  IF v_event_record.genres IS NOT NULL AND array_length(v_event_record.genres, 1) > 0 THEN
    ...
    LOOP
      INSERT INTO user_genre_interactions (
        ...
        genre,
        interaction_type,
        interaction_count,  -- Use interaction_count instead of interaction_strength
        artist_names,       -- Add artist context
        source_entity_type,
        source_entity_id,
        metadata,
        occurred_at
        ...
        v_genre,
        'interest',
        1,                  -- Count of 1 for this interaction
        ARRAY[v_event_record.artist_name],  -- Add artist name to context
        'event_interest',
        NEW.jambase_event_id,
        jsonb_build_object(
          'event_title', v_event_record.title,
      ...
      DO UPDATE SET
        interaction_count = user_genre_interactions.interaction_count + 1,  -- Increment count
        artist_names = CASE 
          WHEN NOT (v_event_record.artist_name = ANY(user_genre_interactions.artist_names))
          THEN user_genre_interactions.artist_names || ARRAY[v_event_record.artist_name]
          ELSE user_genre_interactions.artist_names
        END,
        metadata = jsonb_build_object(
          'event_title', v_event_record.title,
          'artist_name', v_event_record.artist_name,
