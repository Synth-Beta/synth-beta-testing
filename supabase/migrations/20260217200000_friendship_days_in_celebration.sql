-- ============================================
-- Add friendship_days + geo-filtered suggested events
-- ============================================
-- friendship_days: from user_relationships.updated_at when status='accepted'.
-- Used for: "You've been friends with _____ for X days!"
-- Geo filter: when either user has location, restrict suggested events to ~50 miles.
-- ============================================
...
BEGIN
  ...
  -- When the friendship was established (updated_at when status became 'accepted')
...
COMMENT ON FUNCTION public.get_new_friend_celebration_data(uuid) IS 'Returns events attended, shared genres (with match_pct), shared artists/venues, suggested events (geo-filtered when location known), friendship_days.';
