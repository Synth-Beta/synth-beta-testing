-- ============================================================
-- Fix get_personalized_feed_v3 for external IDs normalization
-- ============================================================
-- This is a NEW migration that fixes the function to use external_entity_ids
-- The original function in 20251222000000_update_personalized_feed_v3_photos.sql
-- references dropped columns (artist_jambase_id_text, venue_jambase_id_text)
-- ============================================================

BEGIN;

-- Drop and recreate the function with fixed event_candidates CTE
...
BEGIN
  ...
  artist_follows AS (
    SELECT
      ...
      lower(trim(COALESCE(eei.external_id, a.jambase_artist_id))) AS artist_id_text
    FROM artist_follows af
    JOIN artists a ON a.id = af.artist_id
    LEFT JOIN public.external_entity_ids eei 
      ON eei.entity_uuid = af.artist_id 
  ...
  -- Step 2A: Event Candidates (FIXED - uses external_entity_ids for external ID resolution)
  event_candidates AS (
    SELECT
      e.id AS event_id,
      e.title,
...
'Unified personalized feed v3 with comprehensive photo resolution. Updated for 3NF schema - uses external_entity_ids table for external ID resolution. Photo priority: event_media_url > images JSONB > media_urls > reviews';

GRANT EXECUTE ON FUNCTION public.get_personalized_feed_v3(UUID, INT, INT, NUMERIC, NUMERIC, NUMERIC) TO authenticated;
