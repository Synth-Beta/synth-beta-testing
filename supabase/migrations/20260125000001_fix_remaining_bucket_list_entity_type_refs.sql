-- ============================================
-- FIX REMAINING BUCKET_LIST ENTITY_TYPE REFERENCES
-- ============================================
-- This migration fixes any remaining references to bucket_list.entity_type
-- which was removed in favor of the entities table join
-- ============================================
...
-- Then drop all functions that might have old references to ensure clean recreation
DROP FUNCTION IF EXISTS public.search_bucket_list(UUID, TEXT, INT);
DROP FUNCTION IF EXISTS public.notify_bucket_list_new_event();
DROP FUNCTION IF EXISTS public.update_preference_signals_on_bucket_list_add();
DROP FUNCTION IF EXISTS public.backfill_bucket_list_preference_signals();
DROP FUNCTION IF EXISTS public.calculate_bucket_list_starter(UUID);

-- Ensure search_bucket_list function uses entities table
...
-- Ensure notify_bucket_list_new_event uses entities table (re-apply fix to be safe)
...
-- ============================================
-- STEP 3: ENSURE update_preference_signals_on_bucket_list_add USES NEW SCHEMA
-- ============================================
-- Re-apply the fix to ensure this function uses entities table
...
-- ============================================
-- STEP 4: FIX calculate_bucket_list_starter FUNCTION
-- ============================================
-- This function is called during review insert and still references bl.entity_type
...
COMMENT ON FUNCTION public.calculate_bucket_list_starter IS 'Calculates bucket list starter achievement progress (updated for new entity schema)';

-- ============================================
-- STEP 5: RECREATE TRIGGERS
-- ============================================

-- Recreate trigger for notify_bucket_list_new_event on events table
CREATE TRIGGER trigger_notify_bucket_list_new_event
  AFTER INSERT ON public.events
  FOR EACH ROW
  EXECUTE FUNCTION public.notify_bucket_list_new_event();

-- Recreate trigger for update_preference_signals_on_bucket_list_add on bucket_list table
...
-- ============================================
-- STEP 5: FORCE REFRESH ALL FUNCTION DEFINITIONS
-- ============================================
-- Explicitly refresh function definitions to clear any cached versions
-- This ensures PostgreSQL uses the new function definitions

-- Force PostgreSQL to recompile all functions that might reference bucket_list
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- Find and refresh any functions that might have cached definitions
    FOR func_record IN
        ...
        AND (
            proname LIKE '%bucket%' 
            OR proname LIKE '%preference%'
            OR proname LIKE '%notify%'
        )
    LOOP
        -- Force recompilation by doing a dummy ALTER
        BEGIN
...
-- ============================================
-- STEP 6: DIAGNOSTIC - Find any remaining references
-- ============================================
-- This will help identify if there are any other functions/views that still reference bl.entity_type
-- Run this query manually in Supabase SQL Editor if errors persist:
/*
SELECT 
    routine_schema,
    routine_name,
