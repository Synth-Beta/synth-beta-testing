-- ============================================================
-- User-created artists: allow reviews when artist is not in artists table
-- ============================================================
-- 1. Create user_created_artists table (users can insert their own)
-- 2. Add reviews.user_created_artist_id FK (optional, when artist not in catalog)
-- 3. Constraint: when event_id is null, exactly one of artist_id or user_created_artist_id
-- 4. Update unique index for (user, artist, venue) to use either artist source
-- 5. Update triggers to resolve artist name from either table
-- ============================================================

BEGIN;

-- ============================================================
-- 1. Create user_created_artists table
-- ============================================================
CREATE TABLE IF NOT EXISTS public.user_created_artists (
...
CREATE INDEX IF NOT EXISTS idx_user_created_artists_user_id ON public.user_created_artists(user_id);
CREATE INDEX IF NOT EXISTS idx_user_created_artists_name ON public.user_created_artists(name);

COMMENT ON TABLE public.user_created_artists IS 'User-supplied artist names when the artist is not in the main artists table. Used by reviews via user_created_artist_id.';
COMMENT ON COLUMN public.user_created_artists.user_id IS 'User who created this artist record (owner).';
COMMENT ON COLUMN public.user_created_artists.name IS 'Display name of the artist.';

ALTER TABLE public.user_created_artists ENABLE ROW LEVEL SECURITY;

-- Anyone can read (so review cards can show artist name)
CREATE POLICY "user_created_artists_select"
  ON public.user_created_artists FOR SELECT
  USING (true);

-- Users can insert their own
CREATE POLICY "user_created_artists_insert_own"
  ON public.user_created_artists FOR INSERT
  WITH CHECK (auth.uid() = user_id);

-- Users can update their own
CREATE POLICY "user_created_artists_update_own"
  ON public.user_created_artists FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

-- Users can delete their own
CREATE POLICY "user_created_artists_delete_own"
  ON public.user_created_artists FOR DELETE
  USING (auth.uid() = user_id);

-- Allow authenticated users to insert (RLS will restrict to own user_id)
GRANT SELECT, INSERT, UPDATE, DELETE ON public.user_created_artists TO authenticated;

-- ============================================================
-- 2. Add user_created_artist_id to reviews
-- ============================================================
ALTER TABLE public.reviews
  ADD COLUMN IF NOT EXISTS user_created_artist_id UUID REFERENCES public.user_created_artists(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_reviews_user_created_artist_id ON public.reviews(user_created_artist_id) WHERE user_created_artist_id IS NOT NULL;

COMMENT ON COLUMN public.reviews.user_created_artist_id IS 'Optional: when artist is not in artists table, reference a user-created artist. Exactly one of artist_id or user_created_artist_id when event_id is null.';

-- ============================================================
-- 3. Constraint: when event_id is null, exactly one artist source
-- ============================================================
ALTER TABLE public.reviews
...
ALTER TABLE public.reviews
  ADD CONSTRAINT reviews_artist_or_user_created_check CHECK (
    ...
    (
      ...
      OR
      (artist_id IS NULL AND user_created_artist_id IS NOT NULL)
    )
  );
...
-- ============================================================
DROP INDEX IF EXISTS public.reviews_user_id_artist_id_venue_id_unique;

CREATE UNIQUE INDEX reviews_user_id_artist_or_custom_venue_id_unique
  ON public.reviews(user_id, COALESCE(artist_id, user_created_artist_id), venue_id)
  WHERE event_id IS NULL AND is_draft = false
    AND COALESCE(artist_id, user_created_artist_id) IS NOT NULL
    AND venue_id IS NOT NULL;

COMMENT ON INDEX reviews_user_id_artist_or_custom_venue_id_unique IS
  'One published review per user per artist+venue when event_id is null. Artist can be from artists or user_created_artists.';

-- ============================================================
-- 5. Update capture_review_music_data: artist from review when no event
-- ============================================================
...
BEGIN
  ...
  -- Resolve artist: from event when event_id set, else from review (artist_id or user_created_artist_id)
  ...
  ELSE
    -- Review without event: artist from catalog or user-created
  ...
  -- user_preference_signals (v5): artist signal (genre from artists_genres when catalog artist)
  ...
  -- user_preference_signals (v5): genre signals from review genres
...
COMMENT ON FUNCTION public.capture_review_music_data() IS
  'Captures music metadata from reviews. Resolves artist from event, or from review.artist_id / review.user_created_artist_id when event_id is null. Writes to user_artist_interactions, user_genre_interactions, user_song_interactions and user_preference_signals.';

-- ============================================================
-- 6. Update scene progress: match user_created_artist_id when event_id is null
-- ============================================================
...
COMMENT ON FUNCTION public.auto_update_scene_progress() IS
  'Auto-update scene progress when reviews or passport entries change. Handles reviews with event_id NULL via artist_id or user_created_artist_id and venue_id.';

-- Update calculate_scene_progress to count reviews that use user_created_artist_id
-- (scene_participants.artist_id is UUID from artists table; user_created_artists are not in scene_participants,
-- so we only count catalog artist matches for user_created_artist_id as 0 - no schema change to scene_participants)
-- So the existing UNION that uses r.artist_id for event_id IS NULL is correct; we add a third UNION for user_created_artist_id
-- only if we later add scenes that reference user_created_artists. For now scene_participants.artist_id references artists(id),
-- so user-created artists do not match any scene participant. No change to calculate_scene_progress needed.

-- Optional: updated_at trigger for user_created_artists
...
DROP TRIGGER IF EXISTS trigger_user_created_artists_updated_at ON public.user_created_artists;
CREATE TRIGGER trigger_user_created_artists_updated_at
  BEFORE UPDATE ON public.user_created_artists
