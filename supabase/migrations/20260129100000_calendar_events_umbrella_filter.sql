-- ============================================================
-- Path-based genre filter for calendar events
-- When p_umbrella_slug is set, filter via get_genres_under_umbrella + events_genres
-- ("Show me all Rock events" = events under rock path)
-- ============================================================
...
BEGIN
  ...
  IF p_latitude IS NOT NULL AND p_longitude IS NOT NULL AND p_radius_miles IS NOT NULL AND p_radius_miles > 0 THEN
    ...
    SELECT
      bbc.id, bbc.title, bbc.artist_name, bbc.artist_id, bbc.venue_name, bbc.venue_id,
      bbc.event_date, bbc.doors_time, bbc.description, bbc.genres,
      bbc.venue_address, bbc.venue_city, bbc.venue_state, bbc.venue_zip,
      bbc.latitude, bbc.longitude, bbc.ticket_available, bbc.price_range, bbc.ticket_urls,
      bbc.setlist, bbc.tour_name, bbc.event_media_url, bbc.created_at, bbc.updated_at,
...
COMMENT ON FUNCTION public.get_calendar_events(NUMERIC, NUMERIC, NUMERIC, TIMESTAMPTZ, TEXT[], INT, TEXT, INT) IS
  'Calendar events with optional path-based genre filter. When p_umbrella_slug is set, filters via get_genres_under_umbrella + events_genres.';
