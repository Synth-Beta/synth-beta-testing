-- ============================================
-- Apply events attended together fix (Path B: event_id IS NULL)
...
BEGIN
  ...
  WHERE ur.relationship_type = 'friend' AND ur.status = 'accepted'
    AND ((ur.user_id = v_current_user_id AND related_user_id = p_friend_id)
         OR (ur.user_id = p_friend_id AND related_user_id = v_current_user_id));
