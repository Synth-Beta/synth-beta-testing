-- ============================================
-- COMPLETE ON CONFLICT CONSTRAINT ERROR FIX
-- ============================================
-- Run this entire script in your Supabase database to fix the 42P10 error
-- This addresses all root causes of the ON CONFLICT constraint issues

-- Step 1: Fix the auto_claim_creator_events trigger function
...
-- Step 2: Fix the trigger_event_creation_analytics function
...
-- Step 6: Fix any other functions that might have ON CONFLICT issues
-- Update the set_user_interest function to be more robust
...
-- Step 8: Verify the fix by checking constraint names
SELECT 
    tc.constraint_name,
    tc.constraint_type,
    tc.table_name,
...
WHERE tc.table_name IN ('jambase_events', 'event_claims', 'user_jambase_events')
AND tc.table_schema = 'public'
AND tc.constraint_type = 'UNIQUE'
ORDER BY tc.table_name, tc.constraint_name;

-- Step 9: Test the functions exist
SELECT 
    routine_name,
    routine_type,
    data_type
...
AND routine_name IN ('safe_upsert_jambase_event', 'auto_claim_creator_events', 'trigger_event_creation_analytics', 'set_user_interest')
ORDER BY routine_name;

-- Success message
DO $$
BEGIN
    RAISE NOTICE '✅ ON CONFLICT constraint error fix applied successfully!';
    RAISE NOTICE '✅ All trigger functions updated with proper conflict handling';
    RAISE NOTICE '✅ Safe upsert function created for jambase_events';
    RAISE NOTICE '✅ Constraint names verified and fixed';
    RAISE NOTICE '✅ Ready to test event creation in EventReviewForm';
END $$;
